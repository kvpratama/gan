...
class DCGAN(object):
  ...
  def train(self, config):
    ...
    self.writer = SummaryWriter("./logs/" + self.dataset_name , self.sess.graph)
    ...
    for epoch in xrange(config.epoch):
        ...
        for idx in xrange(0, batch_idxs):
            ...
            # Run g_optim if the difference of D and G error is bigger than x # when D loss is 1 point bigger than G, train G # when G loss is 1 point bigger than D, train G
            if errG - (errD_fake + errD_real) > 1:
                print("dfake_loss: %.8f, dreal_loss: %.8f, g_loss: %.8f" % (errD_fake, errD_real, errG))                
                _, summary_str = self.sess.run([g_optim, self.g_sum], feed_dict={ self.z: batch_z })
                #self.writer.add_summary(summary_str, counter)
                errD_fake = self.d_loss_fake.eval({ self.z: batch_z })
                errD_real = self.d_loss_real.eval({ self.inputs: batch_images })
                errG = self.g_loss.eval({self.z: batch_z})
                #print("loss difference is bigger than 1. updated G network" + )
                print("dfake_loss: %.8f, dreal_loss: %.8f, g_loss: %.8f" % (errD_fake, errD_real, errG))
